...
namespace Utility
{
    public class WndProcHooker
    {
        ...
        public static void HookWndProc(Control ctl,
                                       ...
                                       uint msg,
                                       HookedProcInformation.BaseWndProcTiming timing = HookedProcInformation.BaseWndProcTiming.AfterNew)
        {
            ...
            IntPtr hwnd, uint msg, uint wParam, int lParam)
        {
            if (hwndDict.ContainsKey(hwnd))
            {
                ...
                if (hpi.messageMap.ContainsKey(msg))
                {
                    ...
                    if (hpi.timingMap[msg] == HookedProcInformation.BaseWndProcTiming.BeforeNew) {
                        hpi.CallOldWindowProc(hwnd, msg, wParam, lParam);
                    }

                    bool handled = false;
                    int retval = callback(hwnd, msg, wParam, lParam, ref handled);
                    if (handled || hpi.timingMap[msg] == HookedProcInformation.BaseWndProcTiming.BeforeNew) {
                        return retval;
                    }
                }
                ...
                if (hpi.timingMap.ContainsKey(msg))
                {
                    if (hpi.timingMap[msg] == HookedProcInformation.BaseWndProcTiming.AfterNew)
                    {
        ...
        public class HookedProcInformation
        {
            ...
            public enum BaseWndProcTiming
            {
                BeforeNew,
                AfterNew
            }

            public Dictionary<uint, BaseWndProcTiming> timingMap;
            ...
            public HookedProcInformation(Control ctl, Win32.WndProc wndproc)
            {
                ...
                timingMap = new Dictionary<uint, BaseWndProcTiming>();
            }
