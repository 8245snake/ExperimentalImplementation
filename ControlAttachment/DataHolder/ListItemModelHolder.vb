...
Namespace DataHolder
    ...
    ''' <summary>
    ''' 任意のクラスをリストボックスに紐付けるための補助クラス
    ''' </summary>
    ''' <typeparam name="T">紐付けるクラス</typeparam>
    Public Class ListItemModelHolder(Of T As {Class})
        ...
        Public Property Data(index As Integer) As T
            Get
                Return GetRowDataOrNull(_ListBox.Items(index))
            End Get
            Set
                SetRowData(_ListBox.Items(index), Value)
            End Set
        End Property
        ...
        ''' 紐付けるグリッドを指定して初期化する
        ''' </summary>
        ''' <param name="listBox">グリッド</param>
        Public Sub New(listBox As ListBox)
            _Table = New ConditionalWeakTable(Of Object, T)()
            _ListBox = listBox
            AddHandler _ListBox.HandleDestroyed, AddressOf HandleDestroyed
        End Sub
        ...
        Private Sub HandleDestroyed(sender As Object, e As EventArgs)
            ' 後始末
            RemoveHandler _ListBox.HandleDestroyed, AddressOf HandleDestroyed
            _ListBox = Nothing
        End Sub
